...
import {publishTray, useTrayById} from "../../../hooks/trays/traysHooks";
import PublishTrayRequest from "../../../hooks/trays/requests/PublishTrayRequest";

interface PublishTrayModalProps {
  product: ProductModelRow;
  setOpenedModal: (openedModal: boolean) => void;
}

const PublishTrayModal: React.FC<PublishTrayModalProps> = ({
  product,
  setOpenedModal,
}) => {
  const { t } = useTranslation();
  const { data: trayData } = useTrayById(product.id ? product.id : '');
  ...
  async function handlePublishSweet(
    ...
    const publishRequest = new PublishTrayRequest(id, highlight);
    await publishTray(publishRequest);
            ...
            onClick={() =>
              handlePublishSweet(
                ...
                product?.highlight ? product?.highlight : '',
                  trayData?.images ? trayData?.images : [],
              )
            }
            type="button"
...
export default PublishTrayModal;
